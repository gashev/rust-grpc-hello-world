...
generate_source_from_proto_file: build_docker
	docker run -v $$(pwd):/opt $(DOCKER_REGISTRY)/build:latest protoc --rust_out=. --grpc_out=. --plugin=protoc-gen-grpc=/usr/local/cargo/bin/grpc_rust_plugin helloworld.proto
